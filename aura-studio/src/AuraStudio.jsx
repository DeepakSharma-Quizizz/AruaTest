...
async function apiCreateVideo(userPrompt, isPreset = false) {
  const prompt = [
      // Step 1: Focus on reference sources for accurate aura shape and motion
      `Research and integrate visual styles from iconic anime, games, and VFX (such as Dragon Ball, Solo Leveling, Naruto, and Genshin Impact) to replicate the precise aura shapes and effects from these sources. Focus specifically on capturing the dynamics and patterns of these auras, including how they flow, move, and change shape.`,
    
      // Step 2: Generating aura on black background
      ...
      // Step 3: Specifying shape and motion, avoiding default forms
      `Shape & Style: The aura must have a clearly defined and specific shape based on "${userPrompt}".The aura can include dynamic, irregular forms such as energy spirals, lightning bolts, swarming particle clouds, or energy waves. It can be flames, vertical pillars, or spirals, but the focus should be on a unique, varied, and specific shape that conveys the essence of the prompt.`,
    
      // Step 4: Framing and containment
      ...
      // Step 5: Center gap for composition
      ...
      // Step 6: No characters, only aura effects
      ...
      // Step 7: Emphasizing shape and dynamic movement
      ...
      // Optional refinement for complex structures
      `Ensure that all shapes, motions, and effects follow a consistent, repeatable pattern that can be adapted to different prompts. Focus on creating effects that are flexible for integration into various scenes while maintaining the desired aura form.`
...
export default function AuraStudio() {
  ...
  const generateVideoAura = async (prompt, name, color, isPreset = false) => {
    ...
    try {
      const job = await apiCreateVideo(prompt, isPreset);
  ...
  const handleGenerate = () => {
    ...
    if (!text) return;
    setPromptInput('');
    generateVideoAura(text, text, '#a855f7', false);
  };
        ...
        <div className="relative z-40 w-full max-w-lg mt-2 space-y-3">
          ...
          <div className="flex items-center justify-center gap-1.5 flex-wrap">
            {Object.entries(PRESETS).map(([key, { name, color, prompt }]) => (
              <button
                key={key}
                onClick={() => generateVideoAura(prompt, name, color, true)}
                disabled={generating}
